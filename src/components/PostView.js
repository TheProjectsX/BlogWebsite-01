...
const PostView = () => {
  ...
  for (let i = 0; i < PostArray.length; i++) {
    if ((PostArray[i].title === title) && (PostArray[i].id === id)) {
      if (PostArray[i - 1]) {
  ...
  useEffect(() => {
    const importImage = async () => {
      const imagePath = await importImageByName(AuthorData[currentPost.authorId].authorImg);
    ...
    <div className="lg:w-2/3 pb-10">
      <div className="border-b-2 border-gray-300 pb-4">
        ...
        <div className="buttons flex justify-between py-6">
          ...
          >
            <img className="w-6 py-4" src={MailIcon} alt="Share via Mail" />
          </a>
        </div>
      </div>
