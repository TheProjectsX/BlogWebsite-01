  ...
  return (
    <div className="lg:1/3 xl:w-1/4 py-10 lg:pt-0 space-y-10">
      ...
      <div className="recomended">
        <h2 className="text-3xl font-['Acme',sans-serif]">Recomended</h2>
          ...
          {getLastThreeItems(PostArray).map((item, index) => {
            return (
              <div key={index} className="flex gap-4">
                <Link to={`/posts/${item.title}?id=${item.id}`}>
                  <img src={item.titleImg} alt={item.title} className="w-28" />
                </Link>
                <div>
                  <Link to={`/posts/${item.title}?id=${item.id}`}>
      ...
      <div className="archives">
        <h2 className="text-3xl font-['Acme',sans-serif] mb-3">Archives</h2>
            ...
            ([key, value], index) => {
              return (
                <li key={index}>
                  ○&nbsp;&nbsp;&nbsp;
                  <Link
                    to={`/archive/${key.toLowerCase().replaceAll(" ", "+")}`}
      ...
      <div className="categories">
        <h2 className="text-3xl font-['Acme',sans-serif] mb-3">Categories</h2>
        <ul className="archiveData text-lg">
      ...
      <div className="categories">
        ...
        <div className="flex gap-1">
          <button className="bg-primary p-3 hover:bg-black rounded-md">
            <img src={FeedIcon} alt="contact" className="w-5 h-4" />
          </button>
          <button className="bg-primary p-3 hover:bg-black rounded-md">
            <img src={TwitterIcon} alt="contact" className="w-5" />
          </button>
          <button className="bg-primary p-3 hover:bg-black rounded-md">
            <img src={FacebookIcon} alt="contact" className="w-5" />
          </button>
          <button className="bg-primary p-3 hover:bg-black rounded-md">
            <img src={YoutubeIcon} alt="contact" className="h-4" />
          </button>
        </div>
      </div>
    </div>
