...
const PostBySearch = () => {
  ...
  useEffect(() => {
    setCategoryPosts(
      PostArray.filter((item) => (item.title.toLowerCase().includes(searchQuery.toLowerCase()) || item.body.toLowerCase().includes(searchQuery.toLowerCase()) || item.category.some(category => category.toLowerCase().includes(searchQuery.toLowerCase())))).reverse()
    );
  }, [searchQuery]);
  ...
  return (
    <div className="lg:w-2/3 pb-10">
      ...
      {categoryPosts.length === 0 ? <h2 className="text-2xl">No Post Found!</h2> :categoryPosts.slice(0, postCount).map((post, index) => {
      ...
      <div className="flex justify-center lg:pt-8">
        {postCount <= categoryPosts.length && (
          ...
          >
            {isLoading ? (
              <img src={WaitGif} alt="Please Wait" className="w-12" />
            ) : (
