...
const Slider = (props) => {
  ...
  const { count } = props;
  const posts = PostArray.length <= count ? PostArray : PostArray.slice(-count);
  ...
  return (
    <div className="relative mb-20">
      <div className="overflow-x-auto no-scrollbar" ref={sliderDiv} style={{scrollBehavior: "smooth", transition: "scroll-left 0.3s ease-in-out"}}>
        <div
          ...
          {posts.map((item, index) => {
            ...
            return (
              <div
                ...
                key={index}
              >
                <Link to={postUrl} className="">
                  <img src={item.titleImg} alt={item.title} className="" />
                </Link>
                    ...
                    <span>
                      {item.category.map((catItem, index) => {
                        return (
                          <span key={index}>
                            <Link
                              to={`/category/${catItem.toLowerCase()}`}
      ...
      <div className="buttons space-x-1 absolute bottom-6 right-6">
        <button
          className={`bg-primary py-1.5 px-5 text-2xl text-white font-semibold ${scrollPercentage === 100 ? "hover:bg-primary" : " hover:bg-black"}`}
          disabled={scrollPercentage === 100}
          onClick={() => scrollSlider("left")}
        >{`<`}</button>
        <button
          className={`bg-primary py-1.5 px-5 text-2xl text-white font-semibold ${scrollPercentage === sliderWidth ? "hover:bg-primary" : " hover:bg-black"}`}
