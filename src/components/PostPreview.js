  ...
  return (
    <div className="flex flex-col mb-16 sm:flex-row gap-4 sm:gap-8">
      ...
      <div>
        <p className="pb-2 text-sm text-gray-400">
          ...
          <span>
            {category.map((item, index) => {
              return (
              <span key={index}><Link to={`/category/${item.toLowerCase()}`} className="cursor-pointer hover:text-gray-500">{item}</Link>{index !== category.length - 1 ? " / " : ""}</span>
              )
            })}
          </span>
        </p>
        ...
        <Link to={postUrl} className="sm:hidden md:block lg:hidden 2lg:block">
